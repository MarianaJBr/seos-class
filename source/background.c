...
int background_functions(
   ...
   * and its integral => 'integral_fld*/
    ...
    /* mjb: TODO declare rho_fld = 0. at this point? */
    ...
    if (pba->has_scf == _TRUE_) {
        ...
        pvecback[pba->index_bg_rho_scf] = (phi_prime * phi_prime / (2 * a * a) + V_scf(pba, phi)) /
                                          3.; // energy of the scalar field. The field units are set automatically by setting the initial conditions
        pvecback[pba->index_bg_p_scf] =
                (phi_prime * phi_prime / (2 * a * a) - V_scf(pba, phi)) / 3.; // pressure of the scalar field
    ...
    /* mjb TODO check that the SEOS component is correctly added at this step! */
    /* mjb TODO at what time is this calculated?*/
    ...
    if (return_format == pba->long_info) {
        ...
        /*mjb: added Omega_fld here */
        pvecback[pba->index_bg_Omega_fld] = rho_fld / rho_tot;
    ...
    if (pba->background_verbose > 0) {
        ...
        if (pba->N_ncdm > 0) {
           ...
           from the definition of N_eff) */
                rho_nu_rel =
                        56.0 / 45.0 * pow(_PI_, 6) * pow(4.0 / 11.0, 4.0 / 3.0) * _G_ / pow(_h_P_, 3) / pow(_c_, 7) *
...
int background_indices(
    ...
    /* mjb: added this*/ /*TODO chech this is done properly */
    /* - index for Omega_fld */
    class_define_index(pba->index_bg_Omega_fld, _TRUE_, index_bg, 1);
        ...
        *f0 = 1.0 / pow(2 * _PI_, 3) * (1. / (exp(q - ksi) + 1.) + 1. / (exp(q + ksi) + 1.));
        ...
        if (_FALSE_) {
         ...
         and now called param[..] */
            ...
            mixing_matrix[1][0] = pow(
                    fabs(sqrt((1 - square_s12) * square_s13 * square_s23) + sqrt(square_s12 * (1 - square_s23))), 2);
            mixing_matrix[1][1] = pow(
                    fabs(sqrt(square_s12 * square_s23 * square_s13) - sqrt((1 - square_s12) * (1 - square_s23))), 2);
            mixing_matrix[1][2] = pow(fabs(sqrt(square_s23 * (1 - square_s13))), 2);
            mixing_matrix[2][0] = pow(
                    fabs(sqrt(square_s12 * square_s23) - sqrt((1 - square_s12) * square_s13 * (1 - square_s23))), 2);
            mixing_matrix[2][1] = pow(
                    sqrt((1 - square_s12) * square_s23) + sqrt(square_s12 * square_s13 * (1 - square_s23)), 2);
            ...
            *f0 = 0.0;
            for (i = 0; i < 3; i++) {
                ...
                *f0 += mixing_matrix[i][n_ncdm] * 1.0 / pow(2 * _PI_, 3) *
                       (1. / (exp(q - pba->ksi_ncdm[i]) + 1.) + 1. / (exp(q + pba->ksi_ncdm[i]) + 1.));
    ...
    while (pvecback_integration[pba->index_bi_a] < pba->a_today) {
        ...
        if ((pvecback_integration[pba->index_bi_a] * (1. + ppr->back_integration_stepsize)) < pba->a_today) {
            tau_end = tau_start + ppr->back_integration_stepsize /
                                  (pvecback_integration[pba->index_bi_a] * pvecback[pba->index_bg_H]);
            /* no possible segmentation fault here: non-zeroness of "a" has been checked in background_functions() */
        } else {
            tau_end = tau_start + (pba->a_today / pvecback_integration[pba->index_bi_a] - 1.) /
                                  (pvecback_integration[pba->index_bi_a] * pvecback[pba->index_bg_H]);
            /* no possible segmentation fault here: non-zeroness of "a" has been checked in background_functions() */
        }
    ...
    for (i = 0; i < pba->bt_size; i++) {
       ...
       The value of {B} variables in pData are also copied to pvecback.*/
        ...
        pvecback[pba->index_bg_D] =
                pData[i * pba->bi_size + pba->index_bi_D] / pData[(pba->bt_size - 1) * pba->bi_size + pba->index_bi_D];
...
int background_initial_conditions(
  ...
  */
    ...
    if (pba->has_dcdm == _TRUE_) {
        ...
        if (pba->background_verbose > 3)
            printf("Density is %g. a_today=%g. Omega_ini=%g\n", pvecback_integration[pba->index_bi_rho_dcdm],
                   pba->a_today, pba->Omega_ini_dcdm);
    }
    ...
    if (pba->has_dr == _TRUE_) {
       ...
       */
            f = 1. / 3. * pow(a / pba->a_today, 6) * pvecback_integration[pba->index_bi_rho_dcdm] * pba->Gamma_dcdm /
...
int background_derivs(
    ...
    dy[pba->index_bi_rs] =
            1. / sqrt(3. * (1. + 3. * pvecback[pba->index_bg_rho_b] / 4. / pvecback[pba->index_bg_rho_g])) *
            sqrt(1. - pba->K * y[pba->index_bi_rs] * y[pba->index_bi_rs]); // TBC: curvature correction
    ...
    if (pba->has_fld == _TRUE_) {
        ...
        dy[pba->index_bi_rho_fld] =
                -3. * y[pba->index_bi_a] * pvecback[pba->index_bg_H] * (1. + pvecback[pba->index_bg_w_fld]) *
                y[pba->index_bi_rho_fld];
    }
...
double ddV_scf(
        struct background *pba,
        double phi) {
    return ddV_e_scf(pba, phi) * V_p_scf(pba, phi) + 2 * dV_e_scf(pba, phi) * dV_p_scf(pba, phi) +
